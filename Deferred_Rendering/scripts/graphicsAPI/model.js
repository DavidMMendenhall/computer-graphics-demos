...
let buildModelRenderData = (model, device) => {
    ...
    let vertexBuffer = device.createBuffer({usage:GPUBufferUsage.VERTEX | GPUBufferUsage.COPY_DST, size: vertexCount * 3 * BYTES_PER_FLOAT});
    let normalBuffer = device.createBuffer({usage:GPUBufferUsage.VERTEX | GPUBufferUsage.COPY_DST, size: vertexCount * 3 * BYTES_PER_FLOAT});
    let textureBuffer = device.createBuffer({usage:GPUBufferUsage.VERTEX | GPUBufferUsage.COPY_DST, size: vertexCount * 2 * BYTES_PER_FLOAT});
    let indexBuffer = device.createBuffer({usage:GPUBufferUsage.INDEX | GPUBufferUsage.COPY_DST, size: model.indices.length * model.indices.BYTES_PER_ELEMENT});
